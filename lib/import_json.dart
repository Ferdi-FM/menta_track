...
class ImportJson {
  ...
  Future<void> createWeekPlanMapFromJson(
      Map terminMap) async {
    List<List<WeekTileData>> allAppointmentsList = [];
    for (int i = 0; i < terminMap.length; i++) {
      ...
      for (int j = 0; j < termine
          .length; j++) { //Convertiert die vom Therapeuten erstelle Liste in eine Liste aus Termin-Items für den Patienten
      ...
      List<Termin> existingPlan = await databaseHelper.getWeeklyPlan(
          firstWeekDay);
      if (existingPlan.isEmpty) {
        ...
        DateTime endOfWeek = DateTime.parse(firstWeekDay).add(
            Duration(days: 6));
        String startOfWeekString = DateFormat("dd-MM-yyyy").format(
            DateTime.parse(firstWeekDay));
      ...
      allAppointmentsList.add(weekAppointmentList);
    }
